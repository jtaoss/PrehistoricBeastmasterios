你现在负责把《原始文明：聖獸覺醒》的 iOS 渠道接入现有 `shiqixjp_go` 后端并交付 SVN。先完整阅读 `handoff/ios-channel/README_DEPLOY.md`，再检查真实 SVN 工作副本、当前未提交改动和线上版本。旧 `ios-channel/game/` 跑酷副本已退役，不要再部署。不要从压缩包目录直接提交；应把经过审核的文件合并到真实 SVN 工作副本。
...
5. 当前新小游戏源码在 iOS 仓库 `prototypes/emberwild/`，打包结果在 `PrehistoricBeastmaster/Resources/game/`，入口为 `game/index.html`。本渠道交付目录不再附带游戏文件；不得使用旧跑酷目录或旧 ZIP。若需要后端静态托管，先单独确认交付范围。
