(() => {
    ...
    function buyGameItem(itemId) {
        ...
        tone(660, .1, 'triangle', .035);
        tone(920, .14, 'sine', .03, .08);
        renderShop();
        renderStartStats();
    }
    ...
    function renderShop() {
        ...
        if (shopCategory === 'skins') {
            skins.forEach((skin) => {
                ...
                card.style.setProperty('--skin-body', skin.body);
                card.style.setProperty('--skin-head', skin.head);
                card.style.setProperty('--skin-dark', skin.dark);
                card.style.setProperty('--skin-belly', skin.belly);
                ...
                preview.innerHTML = '<span class="mini-tail"></span><span class="mini-dino"></span><span class="mini-snout"></span><span class="mini-belly"></span><span class="mini-leg mini-leg-back"></span><span class="mini-leg mini-leg-front"></span><span class="mini-arm"></span><span class="skin-eye"></span><span class="skin-cheek"></span><span class="skin-crest"></span><span class="skin-spot"></span>';
    ...
    function cheerMenuHero() {
        ...
        heroTapCount += 1;
        tone(420 + heroTapCount % 3 * 80, .09, 'triangle', .025);
        tone(680, .1, 'sine', .018, .06);
        vibrate(10);
        setTimeout(() => heroInteraction.classList.remove('hero-cheer'), 620);
    }
    ...
    function collectMenuAmber(button) {
        ...
        tone(620 + menuAmberCount * 95, .09, 'sine', .025);
        vibrate(8);
        if (menuAmberCount === 3) {
    ...
    function animateLives() {
        lifeHudEl.classList.remove('life-hit');
        void lifeHudEl.offsetWidth;
        lifeHudEl.classList.add('life-hit');
    }
    ...
    function tone(frequency, duration, type = 'sine', volume = .035, delay = 0) {
        ...
        oscillator.frequency.exponentialRampToValueAtTime(Math.max(60, frequency * .78), start + duration);
    ...
    function performJump() {
        ...
        player.vy = -Math.max(570, height * 1.08);
        player.squash = 1;
        jumpHint.hidden = true;
        tone(340, .085, 'triangle', .035);
        tone(520, .07, 'sine', .018, .045);
        burst(player.x + 8, player.y - 3, '#d9c483', 8, 82);
        showComicFx('咻！', player.x + 35, player.y - 68);
    }
    ...
    function jump() {
        ...
        jumpBuffer = .2;
        if (player.grounded || coyoteTimer > 0 || (player.vy > 0 && player.y > groundY - 22)) performJump();
    }
    ...
    function completeLevel() {
        ...
        resultPanel.classList.add('show');
        tone(520, .12, 'triangle', .045);
        tone(720, .14, 'triangle', .04, .1);
        tone(940, .18, 'sine', .035, .21);
        vibrate([20, 25, 20]);
    }
    ...
    function collectPickup(pickup) {
        ...
        tone(pickup.id === 'rush' ? 920 : 720, .13, 'triangle', .04);
        tone(pickup.id === 'rush' ? 1220 : 940, .16, 'sine', .03, .08);
        vibrate([12, 18, 12]);
        setHud();
    }
    ...
    function collectAmberItem(item) {
        ...
        if (amber % 10 === 0 && lives < maxLives) {
            ...
            tone(760, .11, 'sine', .04);
            tone(980, .14, 'sine', .035, .08);
        } else {
            tone(620 + (amber % 5) * 55, .08, 'sine', .035);
        }
        vibrate(12);
    }
    ...
    function update(dt) {
        ...
        player.blink -= dt;
        if (player.blink < -random(1.2, 3.7)) player.blink = .12;
        ...
        for (const item of obstacles) {
            ...
            if ((rushTimer > 0 || invulnerable <= 0) && rectsOverlap(hitBox, obstacleBox)) {
                if (rushTimer > 0) {
                    ...
                    showToast('護盾擋住了這次碰撞！');
                    tone(330, .1, 'square', .035);
                    tone(760, .2, 'sine', .035, .06);
                    vibrate([18, 20, 18]);
                    updateItemDock();
                    break;
                }
                ...
                if (lives <= 0) {
                    ...
                    showToast(`別急，還有 ${lives} 次機會`);
                    tone(155, .13, 'sawtooth', .035);
                    vibrate([25, 25, 45]);
                }
                break;
            }
    ...
    function drawDino() {
        ...
        const legSwing = running ? Math.sin(player.runPhase) * 6 : 2;
        const bodyBounce = running ? Math.abs(Math.sin(player.runPhase)) * 2.4 : 0;
        const squashX = 1 + player.squash * .09;
        const squashY = 1 - player.squash * .07;
        const ink = '#172638';
        ...
        const drawLeg = (legX, swing, color) => {
            const kneeX = legX + swing * .45;
            const footX = legX + swing;
            ctx.lineCap = 'round';
            ctx.beginPath();
            ctx.moveTo(legX, -24);
            ctx.quadraticCurveTo(kneeX, -12, footX, -4);
            ctx.strokeStyle = ink;
            ctx.lineWidth = 14;
            ctx.stroke();
            ...
            ctx.stroke();
            ctx.beginPath();
            ctx.ellipse(footX + 4, -2, 10, 5, -.08, 0, Math.PI * 2);
            fillAndInk(color, 2.5);
        };

        ctx.save();
        ctx.translate(x, y - bodyBounce);

        ctx.globalAlpha = clamp(1 - (groundY - y) / 260, .35, .65);
        ctx.fillStyle = ink;
        ctx.beginPath();
        ctx.ellipse(3, groundY - y + 3, 34, 6, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = invulnerable > 0 && Math.floor(invulnerable * 12) % 2 === 0 ? .32 : 1;
        ctx.rotate(running ? 0 : clamp(player.vy / 3800, -.14, .11));
        ctx.scale(squashX, squashY);

        /* Rounded tail with a bright tip. */
        ctx.beginPath();
        ctx.moveTo(-22, -48);
        ctx.bezierCurveTo(-39, -50, -55, -47, -63, -40);
        ctx.bezierCurveTo(-51, -35, -42, -24, -23, -22);
        ctx.closePath();
        fillAndInk(activeSkin.dark);
        ctx.fillStyle = activeSkin.accent;
        ctx.beginPath();
        ctx.moveTo(-57, -42); ctx.lineTo(-46, -43); ctx.lineTo(-49, -34); ctx.closePath();
        ctx.fill();

        /* Soft back spikes make the silhouette read as a dinosaur. */
        [-18, -4, 10].forEach((spikeX, index) => {
            ctx.beginPath();
            ctx.moveTo(spikeX - 7, -58 + index * -2);
            ctx.lineTo(spikeX, -74 - index * 2);
            ctx.lineTo(spikeX + 8, -57 + index * -1);
            ctx.closePath();
            fillAndInk(activeSkin.accent, 2.5);
        });

        drawLeg(-12, legSwing, activeSkin.dark);

        ctx.beginPath();
        ctx.ellipse(-1, -39, 35, 27, -.04, 0, Math.PI * 2);
        fillAndInk(activeSkin.body, 3.5);

        /* Belly and two small coat markings. */
        ctx.beginPath();
        ctx.ellipse(8, -33, 18, 16, -.18, 0, Math.PI * 2);
        ctx.fillStyle = activeSkin.belly;
        ctx.fill();
        ctx.fillStyle = activeSkin.dark;
        ctx.globalAlpha *= .34;
        ctx.beginPath();
        ctx.ellipse(-17, -47, 5, 4, -.2, 0, Math.PI * 2);
        ctx.ellipse(-8, -53, 4, 3, .25, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = invulnerable > 0 && Math.floor(invulnerable * 12) % 2 === 0 ? .32 : 1;

        drawLeg(13, -legSwing, activeSkin.body);
        ...
        ctx.lineCap = 'round';
        ctx.beginPath();
        ctx.moveTo(17, -45); ctx.quadraticCurveTo(29, -39, 32, -31);
        ctx.stroke();
        ctx.strokeStyle = activeSkin.head;
        ctx.lineWidth = 4;
        ctx.stroke();

        /* Oversized baby-dino head and muzzle. */
        ctx.beginPath();
        ctx.ellipse(23, -62, 29, 26, -.08, 0, Math.PI * 2);
        fillAndInk(activeSkin.head, 3.5);

        ctx.beginPath();
        ctx.roundRect(28, -59, 34, 20, 10);
        fillAndInk(activeSkin.body, 3);

        /* Three tidy crest plates. */
        [[1, -79, 8, -92, 15, -78], [13, -84, 21, -97, 28, -81]].forEach((points) => {
            ctx.beginPath();
        ...
        /* Friendly face: big eye, highlight, cheek, nostril and smile. */
        ctx.beginPath();
        ctx.ellipse(31, -68, 8, 9, 0, 0, Math.PI * 2);
        fillAndInk('#fffdf2', 2.5);
        const blinkHeight = player.blink > 0 ? 1.5 : 6;
        ctx.fillStyle = ink;
        ctx.beginPath();
        ctx.roundRect(31, -71, 4.5, blinkHeight, 2);
        ctx.fill();
        if (player.blink <= 0) {
            ctx.fillStyle = '#fffdf2';
            ctx.beginPath();
            ctx.arc(33, -70, 1.2, 0, Math.PI * 2);
            ctx.fill();
        }
        ctx.globalAlpha *= .42;
        ctx.fillStyle = activeSkin.accent;
        ctx.beginPath();
        ctx.ellipse(41, -53, 6, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = invulnerable > 0 && Math.floor(invulnerable * 12) % 2 === 0 ? .32 : 1;
        ctx.fillStyle = ink;
        ctx.beginPath();
        ctx.arc(54, -55, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = ink;
        ctx.lineWidth = 2.2;
        ctx.beginPath();
        ctx.arc(47, -51, 9, .18, 1.06);
        ctx.stroke();
