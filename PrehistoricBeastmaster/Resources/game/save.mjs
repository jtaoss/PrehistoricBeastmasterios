...
export function ensureOfferPromptState(profile,migrateCompleted=false){
  if(profile.offerPrompts===undefined){
...
export function freshState(legacy={}) {
  ...
  const state={profile:{runs:number(legacy.runs,1e6),best:number(legacy.best,MAX_WAVES),victories:number(legacy.victories,1e6),companions:freshCompanionState(),tutorialDone:legacyComplete,purchaseTransactions:[],offerPrompts:{starterShown:legacyComplete,weeklyShownWeek:legacyComplete?offerWeekKey():''}},camp:{stones:8,buildings:[],position:{x:980,y:960,angle:0},loadout:normalizeLoadout(DEFAULT_LOADOUT)},run:null,lastResult:null};ensureCampProgress(state.camp);return state;
}
export function validateState(state) {
  ...
  if(state.profile.purchaseTransactions!==undefined){
    need(Array.isArray(state.profile.purchaseTransactions)&&state.profile.purchaseTransactions.length<=200);
    const ids=new Set();for(const item of state.profile.purchaseTransactions){need(item&&/^\d{1,40}$/.test(item.transactionId)&&/^[a-z0-9-]{1,40}$/.test(item.offerId)&&num(item.deliveredAt,0,Number.MAX_SAFE_INTEGER)&&!ids.has(item.transactionId));ids.add(item.transactionId);}
  }
  if(state.profile.offerPrompts!==undefined){const prompts=state.profile.offerPrompts;need(prompts&&typeof prompts.starterShown==='boolean'&&typeof prompts.weeklyShownWeek==='string'&&(prompts.weeklyShownWeek===''||/^\d{4}-W\d{2}$/.test(prompts.weeklyShownWeek)));}
...
export class SaveStore {
  ...
  accept(e,raw){this.state=clone(e.state);ensureCompanionState(this.state.profile);ensureOfferPromptState(this.state.profile,true);ensureLoadoutState(this.state.camp);ensureCampProgress(this.state.camp);this.revision=e.revision;this.savedAt=e.updatedAt;this.validRaw=raw;}
  commit(fn){
    ...
    const next=clone(this.state);const result=fn(next);ensureCompanionState(next.profile);ensureOfferPromptState(next.profile,true);ensureLoadoutState(next.camp);ensureCampProgress(next.camp);validateState(next);const raw=encode(next,this.revision+1);
  ...
  recordStoreKitDelivery(snapshot,{transactionId,offerId,deliveredAt=Date.now()}){
    ...
    return this.mutate(s=>{
      ...
      s.run=clone(snapshot);ledger.push({transactionId:String(transactionId),offerId:String(offerId),deliveredAt});
