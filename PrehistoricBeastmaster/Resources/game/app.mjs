...
import { grantOffer, offerAvailable, offerContents, shopCheckoutHTML, shopOffer } from './shop.mjs';
import { NativeStoreKitShop } from './storekit-shop.mjs';
import { Painter, icon } from './art.mjs';
import { SaveStore, SAVE_KEY, decode, encode, ensureCompanionState, ensureLoadoutState, ensureOfferPromptState, offerWeekKey } from './save.mjs';
...
function refreshCampStoreButtons(){
  const badge=$('camp-gift-badge');if(!badge)return;
  ...
  $('camp-gifts').classList.toggle('sold-out',available===0);
  $('camp-gifts').setAttribute('aria-label',available?`限時禮包，${available} 個可購買`:'限時禮包，目前限定商品已購買');
}
...
function sound(kind) {
  ...
  if(kind==='attack'&&sampleTime-lastSound<.11)return;
  ...
  try {
    ...
    const osc = fallbackAudio.createOscillator(), gain = fallbackAudio.createGain();osc.type = type;osc.frequency.setValueAtTime(freq, now);osc.frequency.exponentialRampToValueAtTime(freq * .55, now + length);
    gain.gain.setValueAtTime(.035*volume, now);gain.gain.exponentialRampToValueAtTime(.001, now + length);osc.connect(gain);gain.connect(fallbackAudio.destination);osc.start(now);osc.stop(now + length);
...
function showCamp(options={}){if(onboardingRequired(store.state)&&!practiceRun()){beginOnboarding();return;}clearInput();closeModal();g=null;tutorialUI.render(null);$('landing').hidden=true;$('game').hidden=true;$('route-map').hidden=true;$('camp').hidden=false;syncBackgroundMusic();campUI.enter();refreshSaveUI();window.scrollTo(0,0);scheduleCampPromotions(Boolean(options?.afterTutorial));}
...
function focusCurrentRouteNode(){
  const viewport=$('route-chapter-scroll'),target=document.querySelector('.route-node.active,.route-node.available')||$('route-current-chapter');
  if(!viewport||!target)return;
  viewport.scrollTop=viewport.scrollHeight;
  const frame=viewport.getBoundingClientRect(),point=target.getBoundingClientRect();
  viewport.scrollTop=Math.max(0,viewport.scrollTop+point.top-frame.top-frame.height*.56);
}
function showRouteComingSoon(label){
  const notice=$('route-coming-toast');
  clearTimeout(routeNoticeTimer);
  $('route-status').textContent=`${label}正在製作中，敬請期待。`;
  notice.textContent=`${label} · 新篇章正在製作中，敬請期待`;
  notice.hidden=false;
  sound('ui');
  experience.haptic('selection');
  routeNoticeTimer=setTimeout(()=>{notice.hidden=true;renderRoute();},2600);
}
async function showRoute(origin='camp'){
  ...
  syncBackgroundMusic();
  renderRoute();window.scrollTo(0,0);requestAnimationFrame(focusCurrentRouteNode);$('route-back').focus();
}
function leaveRoute(){
...
function renderShopSheet(){
  $('shop-sheet')?.remove();
  $('modal').classList.toggle('shop-open',['merchant','promotion'].includes(modalKind)&&!!shopCheckout);
  if(!['merchant','promotion'].includes(modalKind)||!shopCheckout)return;
  $('modal').insertAdjacentHTML('beforeend',shopCheckoutHTML(shopCheckout.offer,shopCheckout.phase,shopCheckout.orderId,spriteIcon('shop-amber-ingot','shop-ingot-art')||spriteIcon(shopCheckout.offer.art,'shop-pack-art'),shopCheckout.detail));
  $('shop-sheet')?.querySelector('button')?.focus();
}
function openShopCheckout(id){
  const offer=shopOffer(id);if(!offer||!['merchant','promotion'].includes(modalKind))return;
  if(!offerAvailable(offer,store.state.profile.purchaseTransactions)){notify(offer.limit==='weekly'?'本週禮包已購買':'新手禮包已購買');return;}
  shopCheckout={offer,phase:'confirm',orderId:'',detail:''};
  renderShopSheet();
}
function closeShopCheckout(){
  if(shopCheckout?.phase==='pending')return;
  shopCheckout=null;renderShopSheet();
  if(modalKind==='merchant')$('modal').querySelector('[data-market-tab][aria-pressed="true"]')?.focus();
  else if(modalKind==='promotion'){
    if(currentPromotion==='gift-center'){$('modal-content').innerHTML=giftCenterContent();refreshCampStoreButtons();}
    $('modal').querySelector('[data-shop-offer]:not([disabled])')?.focus();
  }
}
...
async function showCampPromotions(epoch,kinds,week){
  ...
  try{
    ...
    if(epoch!==promotionEpoch||$('camp').hidden||modalKind||working||saveFailed)return;
    showGiftCenter(true);
  }catch(error){if(epoch===promotionEpoch&&!$('camp').hidden)saveFailure(error);}
}
function scheduleCampPromotions(){
  cancelCampPromotions();currentPromotion='';
  if(!store.state.profile.tutorialDone||!store.state.run||saveFailed)return;
  const prompts=ensureOfferPromptState(store.state.profile),week=offerWeekKey(),transactions=store.state.profile.purchaseTransactions;
  const kinds=[];
...
function closePromotion(){
  if(shopCheckout?.phase==='pending')return;
  shopCheckout=null;currentPromotion='';closeModal();
}
async function beginShopCheckout(){
  ...
  if(!offerAvailable(checkout.offer,store.state.profile.purchaseTransactions)){shopCheckout={...checkout,phase:'error',detail:'此限定禮包已購買，沒有再次發起付款。'};renderShopSheet();return;}
  if(!runId||!shoppablePhase(phase)){shopCheckout={...checkout,phase:'error',detail:'目前遠征無法接收商品，未發起付款。'};renderShopSheet();return;}
  ...
  try{
    ...
    accountSession.accept(authStatus);refreshAccountUI();
    ...
    saveFailed=false;refreshSaveUI();if(g){updateHUD();renderHand();}
    const note=$('market-message');if(note)note.textContent=shopCheckout.detail;
  }catch(error){
...
function showMarket(tab=marketTab,message=''){
  ...
  renderShopSheet();
  if(tab==='shop'){const panel=$('modal').querySelector('.modal-panel'),grid=$('modal').querySelector('.shop-grid');if(panel)panel.scrollTop=0;if(grid)grid.scrollLeft=0;}
}
...
async function openCampStore(kind='recharge'){
  ...
  try{
    ...
    saveFailed=false;ready=true;
  }catch(error){saveFailure(error);}finally{working=false;}
  if(!ready)return;
  refreshSaveUI();
  if(kind==='gifts'){
    cancelCampPromotions();g=null;showGiftCenter();
  }else{
    g=Expedition.restore(store.state.run);showMarket('shop','直接選擇需要的遠征補給；實際金額與幣別以 App Store 系統付款頁為準。');
  }
}
...
function updateHUD() {
  ...
  $('merchant').disabled=!!g.tutorial?.reward;$('merchant').textContent=g.tutorial?.reward?'先領取獎勵':'找商人 ↗';
  $('shop-pay').disabled=!!g.tutorial?.reward;$('shop-pay').hidden=!!g.tutorial?.reward;
  ...
  if($('game').dataset.phase!==g.phase){$('game').dataset.phase=g.phase;if(g.phase==='wave')setBattleDeck(false);}
  ...
  for(const [id,key,label] of [['volley','volleyCD','齊射 · J'],['shock','shockCD','震擊 · K']]){
    ...
    $(`${id}-label`).textContent=!carried?'未攜帶':remaining>0?`${remaining.toFixed(1)}s`:controlLabel(`${label.split(' · ')[0]}${touchControls()?'':` L${level}`}`,id==='volley'?'J':'K');
  }
  ...
  $('deck-hint').textContent = selected !== null ? (touchControls()?'點空地使用 · 再點卡牌取消':'已選卡：點空地使用，Esc 取消') : g.loadout?.legacy?'拖卡使用 · 顯示持有張數':'只顯示本次出征攜帶卡';
  ...
  if(mapEvent){const def=MAP_EVENT_DEFS[mapEvent.type];$('event-interact-icon').textContent=def.icon;$('event-interact-name').textContent=def.name;$('event-interact-action').textContent=controlLabel(g.mapEventPrompt(mapEvent),'E');eventButton.setAttribute('aria-label',`${def.name}，${g.mapEventPrompt(mapEvent)}`);}
...
async function finishTutorial(skip=false){
  ...
  try{
    ...
    if(wasMandatory){showCamp({afterTutorial:true});campUI.message('第一關獎勵已保存！先找行商，用木材與獸骨購買建造卡，再從山口繼續遠征。不需要購買付費禮包。');return;}
...
function handleEvents() {
  for (const ev of g.consumeEvents()) {
    ...
    else if(ev.type==='skill'){notify(ev.name);sound('skill');experience.haptic('medium');painter.shake=2;}
...
function frame(t) {
  ...
  if(!shellAppActive){lastTime=t;return;}
  campUI?.frame(t);
  syncBackgroundMusic();
...
function setBattleDeck(open){
  if(!open){cancelDrag();selected=null;}
  $('game').classList.toggle('deck-open',open);
  $('toggle-deck').setAttribute('aria-expanded',String(open));
  $('toggle-deck').textContent=open?'收起 ▾':'建造 ▴';
}
$('toggle-deck').addEventListener('click',()=>{if(modalKind||!g)return;setBattleDeck(!$('game').classList.contains('deck-open'));renderHand();});
$('battle-view').addEventListener('click',()=>{
  if(modalKind||!painter)return;
  cancelDrag();selected=null;painter.overview=!painter.overview;painter.resize();renderHand();
  $('battle-view').setAttribute('aria-pressed',String(painter.overview));
  $('battle-view').textContent=painter.overview?'近景':'全景';
});
$('hand').addEventListener('pointerdown', e => {
  ...
  if (!g.inventory[g.hand[slot]]) { notify('這張卡用完了，休整時找商人購買'); return; }
  e.preventDefault(); const wasSelected=selected===slot; selected = null; g.building = true;
...
for(const event of ['contextmenu','selectstart'])$('game').addEventListener(event,e=>e.preventDefault());
...
$('camp-gifts').addEventListener('click',()=>openCampStore('gifts'));
$('camp-recharge').addEventListener('click',()=>openCampStore('recharge'));
...
$('modal').addEventListener('click', async e => {
  ...
  if(action==='logout-account'){
    ...
    try{
      ...
      await nativeAuth.logout();accountSession.clear();location.href='login-preview.html?status=signed-out';
  ...
  if(action==='camp-move-building'){campUI.movingFrom=Number(e.target.closest('[data-slot]').dataset.slot);closeModal();campUI.message('走到另一塊空地按「互動」安置。建築暫時保留原位；點「取消搬遷」可返回。');}
