...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:process.env.PBM_CHROME_EXECUTABLE||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});try{
 ...
 const errors=[];p.on('pageerror',e=>errors.push(e.message));await p.goto(url+'/?qa=offensive-cards');
 ...
 assert.deepEqual(state.hand,['watchtower','catapult','wall','spring']);assert.deepEqual(state.types,['watchtower','catapult']);assert.deepEqual(state.labels,['獵脊弩台','琥珀投獸器','裂骨牆','潮汐泉']);assert.ok(state.damage>0);assert.equal(state.fits,true);assert.deepEqual(errors,[]);
 await p.screenshot({path:path.join(out,'30-offensive-building-cards-mobile.png'),fullPage:true});
 console.log('PASS generated offensive cards render, deploy and damage enemies on mobile');await context.close();
