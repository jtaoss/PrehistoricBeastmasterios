...
async function setup(browser, options={}){
  ...
  const p=await ctx.newPage();p.on('pageerror',e=>errors.push(e.message));await require('./camp-helpers.cjs').openVeteran(p,url+'/?qa=1');await p.waitForFunction(()=>window.emberwildQA);return{ctx,p};
}
...
(async()=>{
  ...
  try{
    ...
    assert.equal(await p.locator('#weapon').evaluate(el=>el.disabled),true);assert.equal(await p.evaluate(()=>emberwildQA.game.hero.weapon),'spear');
    console.log('PASS keyboard movement and configured weapon lock');
    // Choosing the route node starts the selected stage immediately.
    await p.waitForTimeout(1500);assert.equal(await p.evaluate(()=>emberwildQA.game.phase),'wave');assert.ok(await p.evaluate(()=>emberwildQA.game.enemies.length+emberwildQA.game.spawnQueue.length)>0);
    await p.evaluate(()=>{const g=emberwildQA.game;g.amber=20;g.spawnEnemy('brute',{x:470,y:220});g.spawnEnemy('spitter',{x:175,y:260});emberwildQA.render();});
    ...
    await m.evaluate(()=>{const g=emberwildQA.game;g.amber=20;g.applyLoadout({cards:['torch','wall','spring','hunter'],weapons:['spear'],skills:['volley']});g.placeCard(0,465,325);g.placeCard(1,455,430);g.placeCard(2,260,520);g.startWave();g.spawnEnemy('raptor',{x:220,y:205});g.spawnEnemy('brute',{x:455,y:235});emberwildQA.render();});
