import { CARDS, HIRES, DEPLOY_CARDS, WORLD, clamp, distance } from './engine.mjs';
...
export class Painter {
  ...
  render(g, preview) {
    ...
    const level=clamp(g.phase==='prep'?g.wave+1:g.wave||1,1,8),bg=this.backgrounds[level-1];
    ...
    for(const p of g.projectiles)this.projectile(p);
    ...
    c.restore();
    const boss=g.enemies.find(e=>BOSS_NAMES[e.type]&&e.hp>0);if(boss){c.fillStyle='#193323dd';c.fillRect(140,90,440,35);c.fillStyle=boss.type==='matriarch'?'#b9cf70':boss.type==='charger'?'#e2a765':'#f0a84f';c.fillRect(148,112,424*boss.hp/boss.maxHp,5);text(c,BOSS_NAMES[boss.type],360,106,12,'#ffddad');}
  }
  ...
  base(b,t){const c=this.c;ellipse(c,b.x,b.y+24,61,21,'#132b205c');ellipse(c,b.x,b.y+3,55,42,'#f3b84b17');
    const painted=drawSprite(c,'sacred-egg',b.x,b.y+43,{height:126+(this.reduced?0:Math.sin(t*1.55)*1.1)});
    if(!painted){ellipse(c,b.x,b.y,51,31,'#6b5c35');ellipse(c,b.x,b.y-4,43,25,'#a2915a');ellipse(c,b.x,b.y-7,36,19,'#405239');for(let i=0;i<9;i++){let a=i/9*Math.PI*2;line(c,[[b.x+Math.cos(a)*42,b.y+Math.sin(a)*24],[b.x+Math.cos(a+.6)*48,b.y+Math.sin(a+.6)*27]],'#c8b276',4);}c.save();c.translate(b.x,b.y-17);c.rotate(-.15);ellipse(c,0,0,21,30,'#edf0cf');polygon(c,[[3,-19],[11,-9],[6,0],[-2,-5]],'#b3c798');c.restore();}
    text(c,'聖 獸 卵',b.x,b.y+58,12,'#fff0c8');this.healthbar(b.x,b.y+68,82,b.hp/b.maxHp,'#d9e5a9');}
  ...
  hero(h,t){const c=this.c,pose=this.walkArt.update(h,t,{reduced:this.reduced});
    ...
    c.save();c.translate(21,-10);c.rotate((h.swing>0?-.55:.15));line(c,[[0,-26],[0,25]],'#c5aa73',4);if(h.weapon==='spear')polygon(c,[[0,-43],[-6,-25],[0,-20],[7,-26]],'#f1e5ba','#8e9774');else polygon(c,[[-1,-28],[15,-32],[23,-17],[14,-10],[0,-14]],'#e5d9ac','#92997a');c.restore();c.restore();
  ...
  telegraph(e){const c=this.c;c.save();
    if(e.type==='charger'){
      c.beginPath();c.moveTo(e.x,e.y);c.lineTo(e.lockX,e.lockY);c.strokeStyle='#e9864b32';c.lineWidth=e.r*1.25;c.lineCap='round';c.stroke();c.setLineDash([8,7]);line(c,[[e.x,e.y],[e.lockX,e.lockY]],'#ffc085c7',3);c.setLineDash([]);
    }else{
      const r=e.type==='boss'?(e.bossPhase?118:92):e.type==='matriarch'?24:e.type==='spitter'?17:30;c.beginPath();c.arc(e.lockX,e.lockY,r,0,Math.PI*2);c.fillStyle=e.type==='matriarch'?'#a8cf5d2e':'#e879502b';c.fill();c.strokeStyle=e.type==='matriarch'?'#d8ee8fc0':'#ffad80b3';c.lineWidth=2;c.setLineDash([5,5]);c.stroke();c.setLineDash([]);
      if(e.type==='spitter'||e.type==='matriarch')line(c,[[e.x,e.y],[e.lockX,e.lockY]],e.type==='matriarch'?'#cde68b7d':'#ecc38465',2);
    }c.restore();}
  projectile(p){const c=this.c;if(p.hostile){ellipse(c,p.x,p.y,8,8,'#bad56b');ellipse(c,p.x-2,p.y-2,3,3,'#e1f19a');return;}if(p.kind==='catapult'){const progress=clamp(1-p.life/p.maxLife,0,1),lift=Math.sin(progress*Math.PI)*78;ellipse(c,p.x,p.y+6,11*(1-lift/260),5*(1-lift/260),'#142a205c');c.save();c.translate(p.x,p.y-lift);c.rotate(progress*Math.PI*3);ellipse(c,0,0,11,9,'#e7a632');polygon(c,[[-7,-6],[-1,-11],[8,-7],[10,2],[3,9],[-8,6]],'#efb743','#8b6732');ellipse(c,-2,-3,4,3,'#fff0a2');c.restore();return;}c.save();c.translate(p.x,p.y);c.rotate(Math.atan2(p.vy,p.vx));if(p.kind==='tower-bolt'){line(c,[[-13,0],[9,0]],'#d9c493',3);polygon(c,[[17,0],[7,-5],[9,0],[7,5]],'#f1e4c1');line(c,[[-14,-4],[-7,0],[-14,4]],'#b5a675',2);c.restore();return;}if(drawSprite(c,'spear',0,0,{height:42,anchorY:.5,rotation:Math.PI/2})){if(p.fire){line(c,[[-33,0],[-16,0]],'#ed985a99',5);line(c,[[-28,-2],[-13,0]],'#ffe6a9',2);}c.restore();return;}line(c,[[-19,0],[9,0]],p.fire?'#ffba63':'#dbc597',3);polygon(c,[[17,0],[5,-5],[7,0],[5,5]],p.fire?'#ffe6a9':'#f4ebce');if(p.fire){line(c,[[-35,-3],[-18,0]],'#ec935a88',4);line(c,[[-29,5],[-15,0]],'#ffd17d88',3);}c.restore();}
