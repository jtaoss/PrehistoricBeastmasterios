...
export class TutorialUI{
  constructor(){
    this.panel=$('tutorial');this.last='';this.g=null;this.visible=false;
  ...
  render(g,blocked=false,painter=null){
    ...
    const signature=[visible,step,pending,skill,g?.phase,t?.started,t?.awaiting,forced].join('|');
    ...
    if(step==='attack'&&t.version===2&&!waiting)$('tutorial-protection').textContent='已命中 '+t.attackHits+' / 2 · 放開操作，看看自動攻擊';
    ...
    marker.hidden=!(guiding&&t.version===2&&['move','build'].includes(step));finger.hidden=!(guiding&&['move','skill','build'].includes(step));$('tutorial-path').setAttribute('d','');
    if(guiding){
      ...
      if(['move','skill','build'].includes(step)){
        ...
        if(step==='build')$('tutorial-path').setAttribute('d','M'+from.x+' '+from.y+'L'+to.x+' '+to.y);
      }
    }
    $('tutorial-holes').setAttribute('d',holes.join(''));
  }
}
