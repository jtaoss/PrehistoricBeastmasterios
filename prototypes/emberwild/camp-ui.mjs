...
export class CampUI {
  constructor(store,{onError,onInteract,isPaused}) {
    ...
    for(const b of document.querySelectorAll('[data-walk-to]'))b.addEventListener('click',()=>this.travel(b.dataset.walkTo));
  ...
  enter(){this.clear();this.walk.restore(this.store.state.camp.position);this.ensureFree();this.painter.resize();this.render();this.lastFrame=performance.now();this.lastSave=this.lastFrame;this.movingFrom=null;this.message(this.store.warning||'自由走動，走近行商購買卡牌；空地可建設，山口可出征。');}
