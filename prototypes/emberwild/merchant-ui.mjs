...
export function marketContent(g,tab,message=''){
  ...
  const tabs=[['build','建造卡'],['hire','雇佣卡'],['forge','武器鍛造'],['training','技能成長'],['shop','¥ 晶錠支付']];
  ...
  const shop=tab==='shop'?shopCatalogHTML(shopArt):'';
  const payStrip=tab==='shop'?'':`<button type="button" class="shop-launch" data-market-tab="shop">${spriteIcon('shop-amber-ingot','shop-launch-ingot')}<div><small>PAYMENT</small><b>晶錠支付</b><p>卡包與單卡結帳畫面，從 ¥3.00 起。</p></div><em>打開商店</em></button>`;
  ...
  return `<div class="merchant-banner${tab==='shop'?' shop-banner':''}"><div class="merchant-portrait" aria-hidden="true">${tab==='shop'?spriteIcon('shop-amber-ingot','merchant-art'):spriteIcon('merchant','merchant-art')}<i>${tab==='shop'?'晶錠支付':'荒境行商'}</i></div><div><b>${tab==='shop'?'用晶錠結帳，價格直接寫在按鈕上。':'材料換好牌，搭配由你決定。'}</b><p>${tab==='shop'?'點擊金色支付按鈕打開結帳層。此為介面演示，不會扣款或發貨。':'固定貨架，不抽卡、不刷新。買幾張，就能部署幾次。也可以改用晶錠支付。'}</p></div></div>
  ...
  <p id="market-message" class="market-message" role="status">${message||(tab==='shop'?'選擇卡包後只打開支付演示，材料與背包維持不變。':'開局含旅行補給；材料與卡牌保存在這次遠征。結算後營火石留在永久營地。')}</p>
  ${skillPaths}${forgePath}${shop}${grid}
  <p class="market-footnote">${tab==='shop'?'卡包支付為介面預覽：確認後停在演示完成，不會呼叫原生支付，也不改動存檔。':'行商只供應本次出征配置中的卡牌、武器與主動技能成長；部分貨架由永久營地設施等級解鎖。建造、升級與部署只消耗卡牌；固定貨架不抽取，也沒有戰後隨機三選一。'}</p>`;
}
